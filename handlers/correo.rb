...
class AppAnimalitos < Sinatra::Base
	post '/correo/mandar' do
	     ...
	     begin  
				DB[:correos].insert(:nombres => correo['nombres'], :apellidos => correo['apellidos'], :correo => correo['correo'], :telefono => correo['telefono'], :mensaje => correo['mensaje'], :momento => Time.now)
